...
export async function checkFalBalance(env = process.env, fetchFn = fetch) {
  ...
  for (const url of FAL_BILLING_URLS) {
    ...
    if (!res.ok) {
      ...
      if (json?.error?.type) lastDetail += ` ${json.error.type}`
      continue
    }
    const remainingUsd =
    ...
    return {
      ...
      username: json?.username || json?.user?.username || json?.user?.name || null,
    }
  }
  if (lastStatus === 401) {
...
/** GET-only lock probe. Does not POST / generate / spend. */
export async function probeFalAccount(env = process.env, fetchFn = fetch) {
  const billing = await checkFalBalance(env, fetchFn)
  const key = env.FAL_KEY
  const res = await fetchFn("https://fal.run/fal-ai/flux/dev", {
    method: "GET",
    headers: falAuthHeader(key),
  })
  const text = await res.text()
  const generateLocked = res.status === 403 && /top.?up|locked|insufficient|balance|credit|payment/i.test(text)
  let keyMatch = "UNKNOWN"
  if (generateLocked || (billing.remainingUsd != null && billing.remainingUsd <= 0)) keyMatch = "NO"
  else if (billing.remainingUsd != null && billing.remainingUsd > 0) keyMatch = "YES"
  return {
    ...billing,
    generateLocked,
    keyMatch,
    probeStatus: res.status,
    probeDetail: generateLocked ? "FAL_TOP_UP_REQUIRED" : `model GET HTTP ${res.status}`,
  }
}
...
export async function generateFalStill(promptOrOpts, env = process.env, fetchFn = fetch) {
  ...
  if (planned.route === STILL_ROUTE_SINGLE_REFERENCE || planned.route === STILL_ROUTE_MULTI_REFERENCE) {
    const urls = planned.body.image_urls || (planned.body.image_url ? [planned.body.image_url] : [])
    for (const imageUrl of urls) {
      if (!/^https?:\/\//i.test(String(imageUrl || "")) && !String(imageUrl || "").startsWith("data:image/")) {
        throw referenceAwareFailure(`refusing to POST private path to Fal (${imageUrl})`)
      }
    }
  }
