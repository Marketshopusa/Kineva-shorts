...
export function watchVersionLabel(version) {
  if (version === "minimax") return "Standard (MiniMax H3)"
  if (version === "motion") return "Premium leftover"
  return "Preview Animatic"
}

export function watchVersionOptions(render) {
  const opts = []
  if (render?.hasMinimax) opts.push({ id: "minimax", label: watchVersionLabel("minimax") })
  if (render?.hasMotion) opts.push({ id: "motion", label: watchVersionLabel("motion") })
  if (render?.hasLegacy) opts.push({ id: "legacy", label: watchVersionLabel("legacy") })
  return opts
}

export function watchPlayQuery(version) {
  if (version === "legacy") return "&version=legacy"
  if (version === "motion") return "&version=motion"
  if (version === "minimax") return "&version=minimax"
  return ""
}

export function watchDownloadName(version) {
  if (version === "minimax") return "episode-1-minimax-v1.mp4"
  if (version === "motion") return "episode-1-motion-v1.mp4"
  return "episode-1.mp4"
}
