...
function buildVideoPrompt(spec, elements, { includeStartFrame = false } = {}) {
  const identity = elementPromptPrefix(elements)
  const start = includeStartFrame
    ? "Take @Image1 as composition, wardrobe, and lighting reference for the opening instant, then immediately animate living performance. Do not hold a still photograph. "
    : ""
  const prompt = [
    start + identity + ".",
    `ACTION: ${spec.action}`,
    `PERFORMANCE: ${spec.facialPerformance} ${spec.bodyPerformance}`,
    `CAMERA: ${spec.cameraAction}`,
    `ENVIRONMENT: ${spec.environmentMotion}`,
...
export function planEpisodeMotionShots(screenplay, characters, {
  ...
  const shots = specs.map((spec, index) => {
    ...
    const videoPrompt = buildVideoPrompt(spec, presence.characterElements, { includeStartFrame: false })
    return {
  ...
  const fitted = fitShotsToCap(shots, { usdPerSecond, capUsd })
  for (const shot of fitted.shots) {
    assertNoSplitScreenPrompt(shot.videoPrompt)
    if (shot.sharedSpace) {
      if (!shot.onScreenCharacterIds.includes(2) || !shot.onScreenCharacterIds.includes(4)) {
        // still valid if ids differ in tests without Ivan locked; prompt already forbids split
      }
    }
  }
