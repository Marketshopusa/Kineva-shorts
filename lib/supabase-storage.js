...
export function renderCandidates(seriesId, episodeId, { version } = {}) {
  const minimax = minimaxRenderPath(seriesId, episodeId)
  ...
  if (version === "minimax") return [minimax]
  if (Number(seriesId) === 1 && Number(episodeId) === 1) return [minimax, motion, demo, episodeOne]
  return [minimax, motion, episodeOne, demo]
}
